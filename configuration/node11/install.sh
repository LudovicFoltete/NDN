...
ndnsec-key-gen /ndn/upec/%C1.Router/router11 > router11.key

#generate the certificate for the router key
ndnsec-cert-dump -i /ndn/upec/%C1.Router/router11 > router11.cert

#install the router certificate
ndnsec-cert-install -f router11.cert
...
mv nlsr.conf ~/NLSR/
mv router11.cert ~/NLSR/
mv router11.key ~/NLSR/
