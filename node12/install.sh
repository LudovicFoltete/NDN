...
ndnsec-key-gen /ndn/upec/%C1.Router/router12 > router12.key

#generate the certificate for the router key
ndnsec-cert-dump -i /ndn/upec/%C1.Router/router12 > router12.cert

#install the router certificate
ndnsec-cert-install -f router12.cert
...
mv nlsr.conf ~/NLSR/
mv router12.cert ~/NLSR/
mv router12.key ~/NLSR/
