...
ndnsec-key-gen /ndn/upec/%C1.Router/router14 > router14.key

#generate the certificate for the router key
ndnsec-cert-dump -i /ndn/upec/%C1.Router/router14 > router14.cert

#install the router certificate
ndnsec-cert-install -f router14.cert
...
mv nlsr.conf ~/NLSR/
mv router14.cert ~/NLSR/
mv router14.key ~/NLSR/
