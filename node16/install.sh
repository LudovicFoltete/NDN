...
ndnsec-key-gen /ndn/upec/%C1.Router/router16 > router16.key

#generate the certificate for the router key
ndnsec-cert-dump -i /ndn/upec/%C1.Router/router16 > router16.cert

#install the router certificate
ndnsec-cert-install -f router16.cert
...
mv nlsr.conf ~/NLSR/
mv router16.cert ~/NLSR/
mv router16.key ~/NLSR/
